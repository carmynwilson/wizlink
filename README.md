...
| File | What it is |
|------|------------|
| **`links.json`** | Your links — `"slug": "destination URL"`. **This is the only file you edit day to day.** Think of it as your spellbook: each entry is a spell that sends someone somewhere. |
...
You have three ways to set this up, easiest first.
...
3. Set the build/deploy so it runs `npx wrangler deploy` (Cloudflare usually detects
   this automatically from `wrangler.toml`). Branch: `main`.
...
> domain just for this (e.g. `examp.le`) and point it at their wizlink Worker.
...
plan, but most personal/organization use stays free. You do need to own a domain.
